...
appia_start () {
	...
	if [[ -x ${APPIA}/FIXCTL_start ]];then
		tlog "[INFO] The ${APPIA}/Start_Task.sh is exist and is executable" $LOG	
		su - twse "-c ${APPIA}/Start_Task.sh $APPIA_Parameter > /dev/null "
		exec_status=$?
		if [[ $exec_status -eq "0" ]];then
			tlog "[INFO] Execute the ${APPIA}/Start_Task.sh Success" $LOG	
		else
			tlog "[ERR] Execute the ${APPIA}/Start_Task.sh Failed" $LOG	
		fi

	else
		tlog "[ERR] The ${APPIA}/Start_Task.sh is not exist or not executable" $LOG	
	fi
}
#}}}
