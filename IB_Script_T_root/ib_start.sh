...
iba1p1="0"
iba1p2="0"
TARGETIP="10.204.5.71  10.204.5.73  10.204.5.71"
...
if [[ $NOW_HOST = $TYPE1A ]] || [[ $NOW_HOST = $TYPE1B ]];then
	HOSTA=$TYPE1A
	HOSTB=$TYPE1B
	HSOTA_VL4="10.199.130.146"
	HSOTB_VL4="10.199.130.96"
	IPADDR="10.204.5.146"
# Example type and ip
elif [[ $NOW_HOST = $TYPE2A ]] || [[ $NOW_HOST = $TYPE2B ]];then
	HOSTA=$TYPE2A
	HOSTB=$TYPE2B
	HSOTA_VL4="10.199.130.87"
	HSOTB_VL4="10.199.130.97"
	IPADDR="10.204.5.147"
else
...
#tlog "#==========================ib_ip_conifg...===========================================#" $LOG
		...
		chdev -l ib0 -a ib_adapter=${DEV_IB} -a ib_port=${DEV_IB_PORT} -a state=up -a netaddr=${IPADDR} -a netmask=255.255.255.0 > /dev/null 2>>$LOG
		exec_status=$?
		if [[ $exec_status -eq "0" ]];then
			return 0
		else 
...
#tlog "#==========================swap function...==========================================#" $LOG
	...
	do
		...
		chdev -l ib0 -a ib_adapter=${NEW_IB} -a ib_port=${NEW_IB_PORT} -a state=up -a netaddr=${IPADDR} -a netmask=255.255.255.0 > /dev/null 2>>$LOG
		exec_status=$?
		if [[ $exec_status -eq "0" ]];then
...
#tlog "#==========================remote_check function..===================================#" $LOG
	...
	if [[ ${ALT_DEV} = iba[0-9] ]]; then
		...
		${HADIR}/clRGmove -s 'false'  -m -i -g ${RG_NAME} -n ${HOSTB} > /dev/null 2>>$LOG
		exec_status=$?
		if [[ $exec_status -eq 0 ]]; then
