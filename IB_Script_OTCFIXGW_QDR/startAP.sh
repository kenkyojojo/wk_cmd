...
ib_smon() {
	...
	if [[ -x ${IBMON}/ibsmon_np ]];then
		...
		su - otc "-c ${IBMON}/ibsmon_np /tmp/ib_moni   1     30"
		exec_status=$?
...
ib_start () {
	...
	if [[ -x ${SHELL}/ib_start.sout ]];then
		...
		su - otc "-c nohup ${SHELL}/ib_start.sout > /dev/null 2>&1 &"
		exec_status=$?
		if [[ $exec_status -eq "0" ]];then
			tlog "[INFO] Execute the ${SHELL}/ib_start.sout Success" $LOG	
		else
			tlog "[ERR] Execute the ${SHELL}/ib_start.sout Failed" $LOG	
		fi
...
ctrlm_start () {
	...
	if [[ -x ${CTRLM}/start-ag ]];then
		...
		su - otc "-c ${CTRLM}/start-ag -u otc -p all"
		exec_status=$?
		if [[ $exec_status -eq "0" ]];then
...
APPIA_Parameter=$1
	...
	if [[ -x ${APPIA}/Startup_Task.sh ]];then
		...
		su - otc "-c ${APPIA}/Startup_Task.sh $APPIA_Parameter > /dev/null "
		exec_status=$?
		if [[ $exec_status -eq "0" ]];then
