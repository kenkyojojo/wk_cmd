...
mk_prive_auth(){
	...
	#mkauth dfltmsg="Custom role to run mkiba command  with twse user"  TWSE.app.exec.mkiba
}
#}}}
...
#{{{mk_role
mk_role(){
#	mkrole authorizations=aix.device.manage.create dfltmsg="Custom role to run mkdev command  with twse user" exec.mkdev
#	mkrole authorizations=TWSE.app.exec.mkiba dfltmsg="Custom role to run mkiba command  with twse user" exec.mkiba
	mkrole authorizations=aix.device.manage.change dfltmsg="Custom role to run chdev  command  with twse user" exec.chdev
	mkrole authorizations=aix.ras.error dfltmsg="Custom role to run errlogger  command  with twse user" exec.errlogger
	mkrole authorizations=TWSE.app.exec.clRGmove  dfltmsg="Custom role to run clRGmove command  with twse user" exec.clRGmove 
}
#}}}
...
#{{{ch_user
ch_user(){
#chuser roles=exec.chdev,exec.errlogger,exec.clRGmove,exec.mkiba,exec.mkdev twse
#chuser roles=exec.chdev,exec.errlogger,exec.clRGmove,exec.mkiba twse
		chuser roles=exec.chdev,exec.errlogger,exec.clRGmove twse
}
#}}}

#{{{ls_user
ls_user(){
	tlog "`lsuser -a roles twse`" $LOG
}
#}}}
