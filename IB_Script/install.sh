...
install () {
	...
	do
		if [[ -f $PG ]];then
			...
			cp $PG $IB_DIR
			tlog "chown $TWSE_OWN ${IB_DIR}/${PG}" $LOG
			chown $TWSE_OWN ${IB_DIR}/${PG}
			tlog "chmod 660 ${IB_DIR}/${PG}" $LOG
			chmod 660 ${IB_DIR}/${PG}
		else
	...
	do
		if [[ -f $PG ]];then
			...
			tlog "chown $TWSE_OWN ${IB_MONI_DIR_RS}/${PG}" $LOG
			chown $TWSE_OWN ${IB_MONI_DIR_RS}/${PG}
			tlog "chmod 660 ${IB_MONI_DIR_RS}/${PG}" $LOG
			chmod 665 ${IB_MONI_DIR_RS}/${PG}
		else
			tlog "Please to check the $PG" $LOG
		fi
	done
